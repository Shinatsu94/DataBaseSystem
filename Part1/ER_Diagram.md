...
本文件依據 [`完整資料庫Schema.sql`](../Part2/完整資料庫Schema.sql) 繪製 Mermaid 實體關聯圖。為提升報告與簡報之可讀性，線段中央明確標示 `1 : N`。允許空值之外鍵參照使用虛線表示。

完整欄位、資料型別與限制條件位於 [資料表詳細規格](../Part2/資料表規格/資料表規格索引.md)，含欄位之詳細圖面位於 [詳細實體關聯圖](ER_Diagram_Detailed.md)。
