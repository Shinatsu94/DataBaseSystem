...
|---|---|---|---|---|---|
| `section_id` | `TINYINT UNSIGNED` | PK | `NOT NULL`、節次範圍 `CHECK` | 節次排序及外鍵識別 | 節次數量小且不得為負數，`TINYINT UNSIGNED` 已足夠；`chk_sections_id_range` 將值域限制為第 1 至第 13 節。 |
| `section_name` | `VARCHAR(20)` | UK | `NOT NULL`、`UNIQUE`、名稱格式 `CHECK` | 顯示「第 1 節」等正式名稱 | 名稱長度可因學校制度改變，使用可變長度文字；`chk_sections_name_format` 與唯一限制避免格式錯誤或同名節次。 |
...
節次屬於校內共同主檔，必須由管理員集中維護，不得讓一般使用者自由輸入。

- `section_id`：目前值域為一至十三節。正則：`^(?:[1-9]|1[0-3])$`。資料庫以 `chk_sections_id_range` 拒絕 `0`、`14`、負數與小數。
- `section_name`：格式固定為 `第 n 節`。正則：`^第 (?:[1-9]|1[0-3]) 節$`。資料庫以 `chk_sections_name_format` 檢查此格式，避免出現 `第一節`、`第01節`、`早自習` 等未定義名稱。
- `start_time`：只接受同一天二十四小時制時刻，秒數固定為 `00`。正則：`^(?:[01][0-9]|2[0-3]):[0-5][0-9]:00$`。資料庫以 `TIME(0)` 與 `CHECK` 限制在 `00:00:00` 至 `23:59:59`。
- `end_time`：格式與 `start_time` 相同。正則：`^(?:[01][0-9]|2[0-3]):[0-5][0-9]:00$`，且必須滿足 `start_time < end_time`。此跨欄位限制由資料庫 `chk_sections_range` 保護。
