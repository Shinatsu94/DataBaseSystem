...
- `course_id`：目前採八位數字課程代碼。正則：`^[0-9]{8}$`。此欄位為主鍵，不接受空白、中文、連字號或重複代碼。
...
- `course_name`：只允許中文、英文字母、數字、空白、括號、頓號、冒號、連字號與斜線，長度二至一百二十字。正則：`^[\p{Han}A-Za-z0-9（）()、:：/\- ]{2,120}$`。合法例：`JAVA程式設計(一)`；不接受網址、程式片段或只有符號的名稱。
