...
- `message`：必須為五至五百字，允許中文、英文字母、數字、空白與常用標點。正則：`^[\p{Han}A-Za-z0-9，。；：、,.!?()（）《》「」\s\-_]{5,500}$`。資料庫以 `chk_notifications_message_length` 檢查長度，完整字元集合由輸入層驗證；不得輸入 HTML、JavaScript、URL 追蹤碼或只有空白的訊息。
