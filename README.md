# 教室租用系統：專案與資料庫 Schema 完整說明

文件版本：`6.3`

適用資料庫：`MariaDB 11.4+`

資料庫名稱：`classroom_rental`

本文件為教室租用系統資料庫專案的主要說明，包含完整系統需求、應用情境、實體關聯圖、MariaDB Schema、完整性限制、資料型態依據、角色權限、View、SQL 範例、測試資料、驗證結果與參考資料。

## 目錄

- [文件導覽](#文件導覽)
- [小組成員](#小組成員)
- [題目與應用情境](#題目與應用情境)
- [系統需求說明](#系統需求說明)
- [身分與權限](#身分與權限)
- [ER Diagram](#er-diagram)
- [完整 DB Schema 架構](#完整-db-schema-架構)
- [各實體與資料表設計](#各實體與資料表設計)
- [其他邏輯規則與 Trigger](#其他邏輯規則與-trigger)
- [索引設計](#索引設計)
- [SQL：概念層到關聯式 Schema](#sql概念層到關聯式-schema)
- [View Schema 與呼叫](#view-schema-與呼叫)
- [範例資料](#範例資料)
- [專案結構](#專案結構)
- [執行方式](#執行方式)
- [驗證結果](#驗證結果)
- [參考資料](#參考資料)

## 文件導覽

| 文件或資料夾 | 內容 |
|---|---|
| [Part1：應用情境、需求、完整性限制與 ER Diagram](Part1/Part1_應用情境與使用案例_系統需求_完整性限制_ERDiagram說明.md) | 對映本總攬的「題目與應用情境」、「系統需求說明」、「完整性限制」與「ER Diagram」。 |
| [Part2：完整資料庫 Schema SQL 與舉例說明](Part2/Part2_完整資料庫Schema_SQL與舉例說明.md) | 對映本總攬的「完整 DB Schema 架構」、「各實體與資料表設計」、「SQL：概念層到關聯式 Schema」與「範例資料」。 |
| [Part3：SQL 概念層、預設資料匯入與 View 查詢](Part3/Part3_SQL概念層_匯入預設資料_View查詢說明.md) | 對映本總攬的「SQL：概念層到關聯式 Schema」、「範例資料」、「View Schema 與呼叫」、「執行方式」與「驗證結果」。 |
| [PPT](PPT/) | 期末簡報檔案與簡報目錄。 |
| [網頁實作](網頁實作/) | 預留前端或網頁實作資料夾，目前不放置實作內容。 |
| [組員心得](組員心得/) | 存放組員心得內容。 |
| [備份：完整資料庫](備份/完整資料庫/) | 存放完整資料庫 SQL、Schema 說明、資料表規格、ER 圖與驗證文件備份。 |

## 小組成員

| 學號 | 班級 | 姓名 |
|---|---|---|
| 41243149 | 四資工三甲 | 廖章竹 |
| 41243151 | 四資工三甲 | 劉向榮 |
| 41243154 | 四資工三甲 | 蔡品辰 |
| 41243161 | 四資工三甲 | 羅冠穎 |

## 題目與應用情境

題目：**教室租用系統**

系統整合下列教室使用情境：

1. 學期固定課程安排教室與授課節次。
2. 學生申請專題討論、班會或校內活動空間。
3. 教師申請調課、補課、考試或週期性輔導教室。
4. 管理員審核申請並保存每次審核決策。
5. 系統檢查固定課表與已核准預約是否發生時段衝突。
6. 系統將審核結果寫入通知紀錄，供使用者查詢。

## 系統需求說明

### 功能性需求

功能性需求描述使用者實際需要完成的工作，以及系統應提供的回應。

#### 所有使用者

| 編號 | 使用者需要 | 系統應提供的功能 |
|---|---|---|
| FR-01 | 使用自己的身分進入系統 | 系統辨識登入者是學生、教師或管理員，並只顯示該身分可使用的功能與資料。 |
| FR-02 | 查看可借用的教室 | 使用者可依日期、節次、教室及容納人數查詢場地，並辨識教室是否啟用、是否已有固定課程或核准預約。 |
| FR-03 | 看懂學校的節次時間 | 系統顯示每一節的名稱、開始時刻與結束時刻，讓使用者不必自行換算時間。 |
| FR-04 | 查看自己的申請進度 | 使用者可查詢個人預約，並清楚看到草稿、待審核、審核中、已核准、已拒絕、待補件或已取消等狀態。 |
| FR-05 | 接收處理結果 | 系統在申請狀態改變時建立通知，內容應包含相關預約、處理結果及必要說明；使用者可標記通知為已讀。 |
| FR-06 | 查詢過去的借用紀錄 | 使用者可依日期與狀態查看自己的歷史申請、審核結果及通知，不得看到其他一般使用者的個人資料。 |

#### 學生

| 編號 | 學生需要 | 系統應提供的功能 |
|---|---|---|
| FR-07 | 申請一次性的教室使用 | 學生可選擇教室、日期、開始節次、結束節次並填寫用途，建立屬於自己的待審核預約。 |
| FR-08 | 在審核前修正資料 | 預約仍為待審核時，學生可修改教室、日期、節次或借用原因，也可取消自己的申請。 |
| FR-09 | 確認申請是否可送出 | 系統應檢查必填欄位、節次順序、教室啟用狀態及申請人身分，並以明確訊息說明無法送出的原因。 |

#### 教師

| 編號 | 教師需要 | 系統應提供的功能 |
|---|---|---|
| FR-10 | 使用學生可用的單次預約功能 | 教師可查詢教室、建立單次預約、追蹤狀態、查看審核結果及通知。 |
| FR-11 | 查看授課課程與固定課表 | 教師可查詢課程名稱、學年度、學期、授課教室、星期及節次安排。 |
| FR-12 | 申請固定週期的教室使用 | 教師可指定日期範圍、每週使用日、教室與節次，建立自己的週期性借用申請。 |
| FR-13 | 管理尚未審核的週期性申請 | 教師可修改或取消自己的待審核週期性借用，但不能修改其他教師的資料或自行核准案件。 |

#### 管理員

| 編號 | 管理員需要 | 系統應提供的功能 |
|---|---|---|
| FR-14 | 維護使用者、教室、節次與狀態 | 管理員可新增、查詢、修改及刪除基礎資料，並可停用暫時不能借用的教室。 |
| FR-15 | 維護課程與固定課表 | 管理員可建立課程、指定授課教師、安排教室與節次，並調整既有固定課表。 |
| FR-16 | 集中處理借用申請 | 管理員可查看全部單次與週期性申請，依申請內容進行核准、拒絕、取消、暫停或要求補件。 |
| FR-17 | 說明每一次審核決策 | 管理員處理案件時可填寫審核意見；系統須保存審核人、決策狀態及處理時間，不得覆寫先前歷程。 |
| FR-18 | 掌握教室使用情況 | 管理員可依教室、日期、申請人及狀態查詢使用紀錄，作為場地管理與後續報表依據。 |

#### 系統自動處理

| 編號 | 系統責任 | 系統應執行的功能 |
|---|---|---|
| FR-19 | 防止教室重複分配 | 核准預約或建立固定課表前，系統必須檢查相同教室、日期或星期及節次是否重疊。 |
| FR-20 | 保護資料所有權 | 學生與教師只能讀寫自己的申請與通知；只有管理員能查看及管理全部資料。 |
| FR-21 | 維持資料關聯正確 | 系統不得接受不存在的使用者、教室、節次、課程或狀態，也不得接受結束節次早於開始節次的資料。 |
| FR-22 | 保存完整處理紀錄 | 系統保存申請建立時間、審核時間、通知時間及目前狀態，使每筆案件都能追溯。 |

### 非功能性需求

非功能性需求定義系統功能必須達到的品質程度，以及用來保障品質的技術措施。尚未執行壓力測試或災難復原演練的數值，列為系統驗收目標。

| 編號 | 品質面向 | 品質與驗收目標 | 技術保障方式 |
|---|---|---|---|
| NFR-01 | 易用性 | 使用者應能在不理解資料庫欄位的情況下完成查詢與申請；表單應使用教室名稱、日期、節次及狀態中文名稱，錯誤訊息須說明原因與處理方式。 | 操作介面使用下拉選單、日期選擇器與必填提示；資料庫以 View 提供易於理解的資料範圍，應用程式將資料庫錯誤轉換為使用者可讀訊息。 |
| NFR-02 | 回應效能 | 一般教室查詢、個人預約與未讀通知，在 10 萬筆預約資料的驗收環境中，95% 請求應於 2 秒內回應；新增預約與衝突檢查應於 3 秒內完成。 | 使用複合索引支援教室日期、申請人日期、固定課表與未讀通知查詢；以 `EXPLAIN` 檢查執行計畫，並以測試資料進行基準測試。 |
| NFR-03 | 正確性 | 無效外鍵、重複唯一值、反向日期或節次、錯誤角色及重疊的已核准時段必須全部被拒絕，不得只依賴前端檢查。 | 使用 `PRIMARY KEY`、`FOREIGN KEY`、`UNIQUE`、`NOT NULL`、`DEFAULT`、`CHECK` 與 Trigger，在 MariaDB 層再次驗證。 |
| NFR-04 | 交易可靠性 | 一次操作涉及多筆資料時，必須全部成功或全部取消，不得留下只有預約而沒有審核或通知的部分資料。 | 所有資料表使用 `InnoDB`；應用程式以交易包覆相關 `INSERT`、`UPDATE`，發生錯誤時執行 `ROLLBACK`。 |
| NFR-05 | 存取安全 | 未授權使用者不得讀取或修改他人資料；學生與教師不得自行核准案件；正式密碼不得存放於專案或 SQL 檔案。 | 使用三種 MariaDB Role、`SQL SECURITY DEFINER` View、欄位級 `GRANT`、資料列條件與 Trigger；部署時使用強密碼、加密連線及安全的密碼管理方式。 |
| NFR-06 | 隱私保護 | 一般使用者只能看到本人帳號、預約、審核結果與通知；管理員存取個人資料應限於行政用途。 | 使用 `USER()` 將 MariaDB 登入帳號對應 `users.user_id`，由 View 隔離資料列；一般角色不授予基礎資料表查詢權。 |
| NFR-07 | 可追蹤性 | 每一次申請、審核與通知都必須能確認相關人員、狀態及事件時間；審核歷程不得被新的結果覆蓋。 | 使用獨立的 `booking_reviews` 歷程表；建立、審核與通知時間採 `TIMESTAMP(6)`，保留微秒級排序能力。 |
| NFR-08 | 可用性 | 在正常校園網路與資料庫服務可用時，使用者應可持續查詢及送出申請；單一不合法操作不得造成整個系統停止服務。 | 應用程式應處理資料庫例外並回復單次交易；資料庫連線使用逾時、重試與連線池設定，部署環境監控服務與連線狀態。 |
| NFR-09 | 備份與復原 | 正式環境至少每日備份一次；驗收目標為最多遺失 24 小時資料，並能在 4 小時內恢復資料庫服務。 | 使用 MariaDB 邏輯或實體備份、異地保存及定期還原演練；Schema 與初始化資料由 Git 版本控制保存。 |
| NFR-10 | 相容性與中文資料 | 系統必須正確保存繁體中文、英文、數字及常用符號；日期與時間不得因顯示環境不同而產生錯誤解讀。 | 資料庫使用 `utf8mb4` 與 `utf8mb4_unicode_ci`；鐘面時刻使用 `TIME(0)`、日曆日期使用 `DATE`、事件時間使用 `TIMESTAMP(6)`，連線明確設定時區。 |
| NFR-11 | 可維護性 | 資料庫結構、權限與 View 必須能由單一正式 Schema 重建；欄位名稱、限制名稱及文件應保持一致。 | 使用一份 `完整資料庫Schema.sql` 管理 DDL、Trigger、View 與 Role；每張資料表另有逐欄規格、型態理由及其他邏輯規則。 |
| NFR-12 | 可測試性 | 每次修改 Schema 後，必須能驗證物件數量、每表資料筆數、合法操作、越權操作及衝突案例。 | 提供 10 張資料表各至少 10 筆範例資料、查詢與 View 範例及驗證文件；測試須涵蓋接受案例與拒絕案例。 |
| NFR-13 | 擴充性 | 新增教室、課程、預約與通知時，不應修改既有交易資料結構；常用資料量增加後仍可透過索引與查詢調整維持效能。 | 採主資料與交易資料分離、第三正規化、代理鍵及外鍵設計；狀態集中於 `booking_statuses` 管理，避免在多張交易表重複定義。 |

## 身分與權限

| 身分 | MariaDB Role | 權限範圍 |
|---|---|---|
| 學生 | `classroom_student_role` | 查詢啟用教室、節次、狀態及自己的帳號、預約、審核結果與通知；建立、修改或取消自己的待審核單次預約；更新自己的通知讀取狀態。 |
| 教師 | `classroom_teacher_role` | 繼承學生權限；增加課程與固定課表查詢，以及自己的週期性借用建立、修改與取消權限。 |
| 管理員 | `classroom_admin_role` | 繼承教師權限；管理全部基礎資料、預約、週期性借用、審核紀錄、通知與 View。 |

學生與教師不能冒用其他帳號提出申請，也不能自行將案件改為核准狀態。資料列範圍由 View 控制，狀態與角色規則由 Trigger 再次驗證。

## ER Diagram

```mermaid
erDiagram
    USERS ||--o{ COURSE_INFO : "教授"
    USERS ||--o{ LONG_TERM_BOOKINGS : "提出週期性借用"
    USERS ||--o{ BOOKINGS : "提出單次預約"
    USERS ||--o{ BOOKING_REVIEWS : "執行審核"
    USERS ||--o{ NOTIFICATIONS : "接收通知"

    CLASSROOMS ||--o{ COURSE_TIMES : "安排固定課表"
    CLASSROOMS ||--o{ LONG_TERM_BOOKINGS : "提供週期性借用"
    CLASSROOMS ||--o{ BOOKINGS : "提供單次預約"

    SECTIONS ||--o{ COURSE_TIMES : "定義開始與結束節次"
    SECTIONS ||--o{ LONG_TERM_BOOKINGS : "定義開始與結束節次"
    SECTIONS ||--o{ BOOKINGS : "定義開始與結束節次"

    BOOKING_STATUSES ||--o{ LONG_TERM_BOOKINGS : "定義處理狀態"
    BOOKING_STATUSES ||--o{ BOOKINGS : "定義處理狀態"
    BOOKING_STATUSES ||--o{ BOOKING_REVIEWS : "記錄審核結果"

    COURSE_INFO ||--o{ COURSE_TIMES : "包含固定授課時段"
    COURSE_TIMES ||--o{ BOOKINGS : "選擇性關聯課程時段"
    LONG_TERM_BOOKINGS ||--o{ BOOKINGS : "展開實際借用日期"
    BOOKINGS ||--o{ BOOKING_REVIEWS : "保留審核歷程"
    BOOKINGS ||--o{ NOTIFICATIONS : "產生通知"
```

### 關聯說明

- 所有現有實體關聯均為 `1:N`。
- `bookings.long_term_id`、`bookings.course_time_id` 與 `notifications.booking_id` 可為 `NULL`，因此屬於選擇性參照。
- `bookings` 是核心交易實體；每一筆資料表示某一日實際發生或申請中的教室使用。
- `long_term_bookings` 保存週期性借用規則，不直接取代特定日期的 `bookings`。
- `booking_reviews` 採新增歷程方式保存決策，不覆寫既有審核紀錄。

詳細欄位 ER 圖：

- [Mermaid 詳細 ER 圖](Part1/ER_Diagram_Detailed.md)
- [diagrams.net 可編輯 ER 圖](Part1/ER_Diagram_Detailed.drawio)

## 完整 DB Schema 架構

### 分層設計

| 層級 | 內容 | 專案實作 |
|---|---|---|
| 概念層 | 定義實體、屬性、關聯與基數 | 上方 ER Diagram 與關聯說明 |
| 邏輯層 | 將實體轉換為資料表、主鍵與外鍵 | 10 張關聯式資料表 |
| 實體層 | 指定 MariaDB 型態、索引、Trigger、字元集與儲存引擎 | `完整資料庫Schema.sql` |
| 存取層 | 限制不同角色可讀寫的資料列與欄位 | 10 個 View、3 個 Role、欄位級 `GRANT` |

### 資料表分類

| 分類 | 資料表 | 系統用途 |
|---|---|---|
| 主資料 | `users` | 使用者身分、角色、電子郵件與所屬單位 |
| 主資料 | `classrooms` | 教室代碼、名稱、容量與啟用狀態 |
| 主資料 | `sections` | 校訂節次與每日起訖時刻 |
| 主資料 | `booking_statuses` | 申請生命週期的 10 種狀態 |
| 課程資料 | `course_info` | 學年度、學期、課程與授課教師 |
| 課程資料 | `course_times` | 課程的固定星期、教室與節次 |
| 借用資料 | `long_term_bookings` | 週期性借用規則與處理狀態 |
| 核心交易 | `bookings` | 每一筆實際日期的教室預約 |
| 稽核資料 | `booking_reviews` | 管理員審核決策與時間 |
| 通知資料 | `notifications` | 使用者通知及已讀狀態 |

完整 SQL 集中於單一檔案：

- [完整資料庫 Schema SQL](Part2/完整資料庫Schema.sql)
- [完整 Schema 架構與設計依據](Part2/完整資料庫Schema架構與說明.md)

### 完整性限制

| 類型 | 實作 | 目的 |
|---|---|---|
| 實體完整性 | `PRIMARY KEY` | 每筆資料具有不可重複且不可為空的識別值。 |
| 參照完整性 | `FOREIGN KEY` | 子資料只能參照已存在的父資料。 |
| 值域完整性 | `ENUM`、`CHECK`、無號整數 | 限制角色、狀態、星期、學期、容量、日期與節次範圍。 |
| 唯一性 | `UNIQUE` | 防止電子郵件、節次名稱及狀態代碼重複。 |
| 其他邏輯規則 | Trigger | 驗證教師、管理員、資料所有權、教室啟用狀態與時段衝突。 |
| 存取控制 | View、Role、欄位級 `GRANT` | 實作最小權限原則與個人資料隔離。 |

### 型態設計原則

- `CHAR(8)`：僅用於固定八碼的校內帳號。
- `VARCHAR(n)`：用於長度確實可變的名稱、電子郵件、教室代碼、課程代碼與狀態代碼。
- `TINYINT/SMALLINT UNSIGNED`：用於不允許負數且值域明確的小型數值。
- `BIGINT UNSIGNED AUTO_INCREMENT`：用於會長期累積的交易識別碼。
- `TEXT`：用於借用原因、審核意見及通知內容等不定長度敘述。
- `BOOLEAN`：用於教室啟用與通知已讀狀態。
- `TIME(0)`：保存每日鐘面時刻，精確至秒，不包含日期與時區。
- `DATE`：保存日曆日期，不包含時刻且不執行時區轉換。
- `TIMESTAMP(6)`：保存建立、審核及通知等事件時間，保留六位小數秒並依連線時區轉換。

## 各實體與資料表設計

下表恢復原專案總覽的逐實體導覽方式，並更新為目前 MariaDB Schema 的正式型態、限制、View 與資料量。每個實體名稱均可開啟完整逐欄規格。

| 實體 | 主鍵 | 主要外鍵 | 資料表用途 | 對應 View | 範例資料 |
|---|---|---|---|---|---:|
| [`users`](Part2/資料表規格/01_users.md) | `user_id CHAR(8)` | 無 | 保存學生、教師與管理員身分、電子郵件及所屬單位。 | `vw_users` | 26 |
| [`classrooms`](Part2/資料表規格/02_classrooms.md) | `classroom_id VARCHAR(10)` | 無 | 保存教室代碼、名稱、容量及啟用狀態。 | `vw_classrooms` | 10 |
| [`sections`](Part2/資料表規格/03_sections.md) | `section_id TINYINT UNSIGNED` | 無 | 保存校訂節次名稱及秒級起訖時刻。 | `vw_sections` | 13 |
| [`booking_statuses`](Part2/資料表規格/04_booking_statuses.md) | `status_id TINYINT UNSIGNED` | 無 | 集中管理預約生命週期的 10 種狀態。 | `vw_booking_statuses` | 10 |
| [`course_info`](Part2/資料表規格/05_course_info.md) | `course_id VARCHAR(20)` | `teacher_id` | 保存學年度、學期、課程名稱及授課教師。 | `vw_course_info` | 10 |
| [`course_times`](Part2/資料表規格/06_course_times.md) | `course_time_id BIGINT UNSIGNED` | 課程、教室、起訖節次 | 保存固定課表的星期、教室及節次範圍。 | `vw_course_times` | 10 |
| [`long_term_bookings`](Part2/資料表規格/07_long_term_bookings.md) | `long_term_id BIGINT UNSIGNED` | 申請人、教室、起訖節次、狀態 | 保存日期範圍內的週期性借用規則。 | `vw_long_term_bookings` | 10 |
| [`bookings`](Part2/資料表規格/08_bookings.md) | `booking_id BIGINT UNSIGNED` | 申請人、教室、來源、起訖節次、狀態 | 保存特定日期實際發生或申請中的單次預約。 | `vw_bookings` | 10 |
| [`booking_reviews`](Part2/資料表規格/09_booking_reviews.md) | `review_id BIGINT UNSIGNED` | 預約、審核人、狀態 | 保存不可覆寫的審核決策歷程。 | `vw_booking_reviews` | 10 |
| [`notifications`](Part2/資料表規格/10_notifications.md) | `notification_id BIGINT UNSIGNED` | 收件人、選擇性預約 | 保存通知內容、關聯案件及已讀狀態。 | `vw_notifications` | 10 |

### 各實體欄位摘要

下表提供前端與簡報可直接查看的欄位摘要；完整型態、限制與設計理由仍以各資料表規格文件為準。

| 實體 | 欄位摘要 | 前端常用欄位 |
|---|---|---|
| `users` | `user_id`, `username`, `email`, `role`, `department` | 顯示登入者姓名、身分、系所與聯絡資料。 |
| `classrooms` | `classroom_id`, `classroom_name`, `capacity`, `is_active` | 顯示教室名稱、容量與是否可借用。 |
| `sections` | `section_id`, `section_name`, `start_time`, `end_time` | 顯示節次清單與起訖時間。 |
| `booking_statuses` | `status_id`, `status_code`, `status_name` | 顯示預約狀態文字與狀態篩選選項。 |
| `course_info` | `course_id`, `academic_year`, `semester`, `course_name`, `teacher_id` | 顯示課程名稱、學年學期與授課教師。 |
| `course_times` | `course_time_id`, `course_id`, `classroom_id`, `day_of_week`, `start_section_id`, `end_section_id` | 顯示固定課表的星期、教室與節次範圍。 |
| `long_term_bookings` | `long_term_id`, `applicant_id`, `classroom_id`, `start_date`, `end_date`, `day_of_week`, `start_section_id`, `end_section_id`, `reason`, `status_id`, `created_at` | 顯示週期性借用申請、日期範圍與審核狀態。 |
| `bookings` | `booking_id`, `applicant_id`, `classroom_id`, `long_term_id`, `course_time_id`, `booking_date`, `start_section_id`, `end_section_id`, `reason`, `status_id`, `created_at` | 顯示單次預約日期、教室、節次、用途與目前狀態。 |
| `booking_reviews` | `review_id`, `booking_id`, `reviewer_id`, `status_id`, `comment`, `reviewed_at` | 顯示審核決策、審核意見與處理時間。 |
| `notifications` | `notification_id`, `recipient_id`, `booking_id`, `message`, `is_read`, `created_at` | 顯示通知內容、關聯預約與已讀狀態。 |

### 預約狀態

| 代碼 | 中文名稱 | 使用意義 |
|---|---|---|
| `draft` | 草稿 | 尚未送出審核。 |
| `pending` | 待審核 | 已送出並等待管理員處理。 |
| `under_review` | 審核中 | 管理員正在確認場地、設備或申請資料。 |
| `approved` | 已核准 | 取得教室使用權，納入時段衝突檢查。 |
| `rejected` | 已拒絕 | 申請未通過。 |
| `canceled` | 已取消 | 申請人或管理員取消案件。 |
| `expired` | 已逾期 | 使用日期已過且未取得有效核准。 |
| `completed` | 已完成 | 教室使用與設備確認已完成。 |
| `suspended` | 已暫停 | 因設備、場地或行政原因暫停。 |
| `resubmission_required` | 待補件 | 申請資料不足，須補充後重新送審。 |

只有 `approved` 狀態會占用教室並觸發固定課表與單次預約的完整衝突驗證。

## 其他邏輯規則與 Trigger

| Trigger | 觸發時機 | 驗證內容 |
|---|---|---|
| `trg_course_info_validate_teacher_insert` | 新增課程前 | 授課者必須是教師；教師只能建立指派給自己的課程。 |
| `trg_course_info_validate_teacher_update` | 修改課程前 | 維持授課教師角色與資料所有權。 |
| `trg_course_times_prevent_overlap_insert` | 新增固定課表前 | 阻擋同教室、同星期與重疊節次的固定課表或已核准預約。 |
| `trg_course_times_prevent_overlap_update` | 修改固定課表前 | 修改後仍須符合固定課表與預約衝突限制。 |
| `trg_long_term_validate_insert` | 新增週期性借用前 | 學生不得建立週期性借用；教師只能建立自己的待審核案件。 |
| `trg_long_term_validate_update` | 修改週期性借用前 | 教師只能修改或取消自己的待審核案件。 |
| `trg_bookings_prevent_overlap_insert` | 新增單次預約前 | 驗證申請人、資料所有權、教室啟用狀態及核准時段衝突。 |
| `trg_bookings_prevent_overlap_update` | 修改單次預約前 | 驗證狀態轉換、資料所有權、教室狀態及核准時段衝突。 |
| `trg_reviews_validate_insert` | 新增審核歷程前 | 審核人必須具有管理員身分。 |
| `trg_reviews_validate_update` | 修改審核歷程前 | 修改後的審核人仍必須具有管理員身分。 |

節次重疊判斷採用：

```sql
NEW.start_section_id <= existing.end_section_id
AND NEW.end_section_id >= existing.start_section_id
```

## 索引設計

| 索引 | 欄位 | 查詢用途 |
|---|---|---|
| `idx_bookings_classroom_date` | 教室、日期、起訖節次 | 教室可用性與時段衝突查詢。 |
| `idx_bookings_applicant_date` | 申請人、日期 | 個人預約歷史查詢。 |
| `idx_course_times_classroom_weekday` | 教室、星期、起訖節次 | 固定課表與衝突查詢。 |
| `idx_notifications_recipient_read` | 收件人、已讀狀態 | 個人未讀通知查詢。 |

## SQL：概念層到關聯式 Schema

概念層的「使用者提出預約」關聯，在 SQL 中轉換為 `bookings.applicant_id` 外鍵：

```sql
CREATE TABLE bookings (
  booking_id       BIGINT UNSIGNED AUTO_INCREMENT PRIMARY KEY,
  applicant_id     CHAR(8) NOT NULL,
  classroom_id     VARCHAR(10) NOT NULL,
  booking_date     DATE NOT NULL,
  start_section_id TINYINT UNSIGNED NOT NULL,
  end_section_id   TINYINT UNSIGNED NOT NULL,
  reason           TEXT NOT NULL,
  status_id        TINYINT UNSIGNED NOT NULL,
  created_at       TIMESTAMP(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6),
  FOREIGN KEY (applicant_id) REFERENCES users(user_id),
  FOREIGN KEY (classroom_id) REFERENCES classrooms(classroom_id),
  FOREIGN KEY (status_id) REFERENCES booking_statuses(status_id),
  CHECK (start_section_id <= end_section_id)
) ENGINE=InnoDB;
```

概念層的「預約由管理員審核」關聯，在查詢中透過外鍵連接：

```sql
SELECT
  b.booking_id,
  applicant.username AS applicant_name,
  reviewer.username AS reviewer_name,
  status.status_name,
  br.reviewed_at
FROM booking_reviews AS br
JOIN bookings AS b ON b.booking_id = br.booking_id
JOIN users AS applicant ON applicant.user_id = b.applicant_id
JOIN users AS reviewer ON reviewer.user_id = br.reviewer_id
JOIN booking_statuses AS status ON status.status_id = br.status_id;
```

完整概念層映射與查詢位於 [查詢與 View 範例](Part3/查詢與View範例.sql)。

## View Schema 與呼叫

| View | 對應資料表 | 使用目的 |
|---|---|---|
| `vw_users` | `users` | 一般使用者僅查看本人；管理員查看全部。 |
| `vw_classrooms` | `classrooms` | 一般使用者僅查看啟用教室。 |
| `vw_sections` | `sections` | 提供全校節次定義。 |
| `vw_booking_statuses` | `booking_statuses` | 提供申請狀態代碼與名稱。 |
| `vw_course_info` | `course_info` | 教師與管理員查詢課程資料。 |
| `vw_course_times` | `course_times` | 教師與管理員查詢固定課表。 |
| `vw_long_term_bookings` | `long_term_bookings` | 教師查看自己的週期性借用；管理員查看全部。 |
| `vw_bookings` | `bookings` | 使用者查看及維護自己的預約；管理員查看全部。 |
| `vw_booking_reviews` | `booking_reviews` | 申請人查看相關審核歷程；管理員查看全部。 |
| `vw_notifications` | `notifications` | 使用者查看自己的通知；管理員查看全部。 |

基本呼叫：

```sql
SELECT * FROM vw_bookings ORDER BY created_at DESC;
SHOW CREATE VIEW vw_bookings;
```

學生透過 View 新增自己的待審核預約：

```sql
INSERT INTO vw_bookings(
  applicant_id, classroom_id, booking_date,
  start_section_id, end_section_id, reason, status_id
) VALUES (
  '41243149', 'BGC0508', DATE '2026-07-01',
  5, 6, '暑期專題討論', 1
);
```

## 範例資料

[完整範例資料 SQL](Part3/範例資料.sql) 為每張資料表提供至少 10 筆具體且互相一致的真實範例資料：

| 資料表 | 筆數 |
|---|---:|
| `users` | 26 |
| `classrooms` | 10 |
| `sections` | 13 |
| `booking_statuses` | 10 |
| `course_info` | 10 |
| `course_times` | 10 |
| `long_term_bookings` | 10 |
| `bookings` | 10 |
| `booking_reviews` | 10 |
| `notifications` | 10 |

資料包含具體教室、課程、節次、日期、申請原因、審核意見與通知內容，並符合所有外鍵及完整性限制。

## 專案結構

```text
.
├─ README.md
├─ Part1/
│  ├─ Part1_應用情境與使用案例_系統需求_完整性限制_ERDiagram說明.md
│  ├─ ER_Diagram.md
│  ├─ ER_Diagram_Detailed.md
│  └─ ER_Diagram_Detailed.drawio
├─ Part2/
│  ├─ Part2_完整資料庫Schema_SQL與舉例說明.md
│  ├─ 完整資料庫Schema.sql
│  ├─ 範例資料.sql
│  ├─ 完整資料庫Schema架構與說明.md
│  └─ 資料表規格/
├─ Part3/
│  ├─ Part3_SQL概念層_匯入預設資料_View查詢說明.md
│  ├─ 完整資料庫Schema.sql
│  ├─ 範例資料.sql
│  ├─ 查詢與View範例.sql
│  └─ 驗證說明.md
├─ PPT/
├─ 網頁實作/
├─ 組員心得/
└─ 備份/
   ├─ 完整資料庫/
   └─ 原始分類資料/
```

資料表逐欄規格入口：[10 張資料表詳細規格](Part2/資料表規格/資料表規格索引.md)

## 執行方式

```powershell
mariadb -u root -p < "Part3/完整資料庫Schema.sql"
mariadb -u root -p < "Part3/範例資料.sql"
mariadb -u root -p < "Part3/查詢與View範例.sql"
```

## 驗證結果

本專案已於 `MariaDB 11.4.12` 完成實際執行驗證：

- 成功建立 10 張資料表。
- 成功建立 10 個 Trigger。
- 成功建立 10 個 View。
- 成功建立 3 個 MariaDB Role。
- 每張資料表均成功載入至少 10 筆資料。
- 外鍵、角色、節次範圍、教室啟用狀態與時段衝突限制均可正常執行。

完整測試紀錄：[資料庫驗證說明](Part3/驗證說明.md)

## 參考資料

### 課程指定資料

1. [*Appendix B: Summary of the Database Design*](<Part3/參考資料/Appendix B Summary of the database design.pdf>)。課程指定 PDF。
2. [*Appendix E: Common Data Models*](<Part3/參考資料/Appendix E Common data models.pdf>)。課程指定 PDF。
3. [*Appendix E 20210511*](<Part3/參考資料/Appendix E 20210511.pptx>)。課程指定 PowerPoint。

### MariaDB 官方文件

4. MariaDB Foundation. [CREATE TABLE](https://mariadb.com/docs/server/server-usage/tables/create-table). 查閱日期：2026-06-10。
5. MariaDB Foundation. [Primary Key Constraints](https://mariadb.com/docs/server/architecture/server-constraints/primary-key-constraints). 查閱日期：2026-06-10。
6. MariaDB Foundation. [Foreign Key Constraints](https://mariadb.com/docs/server/architecture/server-constraints/foreign-key-constraints). 查閱日期：2026-06-10。
7. MariaDB Foundation. [CONSTRAINT](https://mariadb.com/docs/server/reference/sql-statements/data-definition/constraint). 查閱日期：2026-06-10。
8. MariaDB Foundation. [CREATE VIEW](https://mariadb.com/docs/server/server-usage/views/create-view). 查閱日期：2026-06-10。
9. MariaDB Foundation. [SHOW CREATE VIEW](https://mariadb.com/docs/server/reference/sql-statements/administrative-sql-statements/show/show-create-view). 查閱日期：2026-06-10。
10. MariaDB Foundation. [Roles Overview](https://mariadb.com/docs/server/security/user-account-management/roles/roles_overview). 查閱日期：2026-06-10。
11. MariaDB Foundation. [GRANT](https://mariadb.com/docs/server/reference/sql-statements/account-management-sql-statements/grant). 查閱日期：2026-06-10。
12. MariaDB Foundation. [Date and Time Data Types](https://mariadb.com/docs/server/reference/data-types/date-and-time-data-types). 查閱日期：2026-06-10。
13. MariaDB Foundation. [TIME](https://mariadb.com/docs/server/reference/data-types/date-and-time-data-types/time). 查閱日期：2026-06-10。
14. MariaDB Foundation. [TIMESTAMP](https://mariadb.com/docs/server/reference/data-types/date-and-time-data-types/timestamp). 查閱日期：2026-06-10。

### 圖表工具

15. Mermaid. [Entity Relationship Diagrams](https://mermaid.js.org/syntax/entityRelationshipDiagram.html). 查閱日期：2026-06-10。
16. JGraph Ltd. [diagrams.net](https://www.diagrams.net/). 查閱日期：2026-06-10。
