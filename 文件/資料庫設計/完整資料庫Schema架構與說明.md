...
> [返回專案總覽](../../README.md) | [完整 Schema SQL](../../資料庫/完整資料庫Schema.sql) | [範例資料](../../資料庫/範例資料.sql)
...
完整逐欄規格位於 [資料表規格索引](../資料表規格/README.md)。
...
| 限制 | 套用資料 |
|---|---|
| `chk_users_id_format` | 學號為八位數字；教師及管理員帳號分別以 `T`、`A` 開頭。 |
...
[`範例資料.sql`](../../資料庫/範例資料.sql) 對 10 張資料表各建立 10 筆具體資料。資料載入使用交易控制，任何一筆違反限制時可避免留下部分完成的資料集。

## 參考資料

完整參考資料及查閱日期統一列於 [專案總覽最後一節](../../README.md#參考資料)。
