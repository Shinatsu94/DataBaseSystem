...
本文件依據 [`schema.sql`](./schema.sql) 繪製 Mermaid 實體關聯圖。為提升報告與簡報之可讀性，線段中央明確標示 `1 : N`。允許空值之外鍵參照使用虛線表示。

完整欄位、資料型別與限制條件位於 [`實體資料表格`](./實體資料表格/README.md)。
...
| `1 : 1` | 否 | 目前無任一實體限制為一對一關聯。 |
| `N : N` | 否 | 目前未設置多對多關聯。若後續新增多對多關聯，應建立中介實體保存雙方外鍵。 |
