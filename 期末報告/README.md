...
- `Part2_schema.sql`：SQLite 完整建表語法、限制條件、索引與防止重複預約的 Trigger。
- `Part2_examples.sql`：範例資料、查詢與衝突測試範例。
- `Part2_驗證說明.md`：已完成的正確資料與錯誤資料測試整理。
- `Part2_專案詳細說明.txt`：完整專案細節、各實體欄位、完整性限制、關聯、Trigger、索引與後續建議。
...
- 固定課表、已核准預約，以及課表與預約之間的交叉檢查，皆使用 Trigger 防止同一教室的時段重疊。
- 待審核案件可以並存，由管理員核准時再次檢查衝突。
