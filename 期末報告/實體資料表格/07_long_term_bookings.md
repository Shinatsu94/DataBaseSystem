...
|---|---|---|---|---|---|---|---|
| 1 | `long_term_id` | `INTEGER` | 是 | PK | 自動編號 | `PRIMARY KEY AUTOINCREMENT` | 長期借用識別碼 |
...
| 6 | `day_of_week` | `INTEGER` | 是 | - | 無 | `NOT NULL`、`CHECK (day_of_week BETWEEN 1 AND 7)` | 每週借用日：`1` 為星期一，`7` 為星期日 |
...
| 10 | `status_id` | `INTEGER` | 是 | FK → `booking_statuses.status_id` | 無 | `NOT NULL`、外鍵參照必須存在 | 目前狀態 |
...
|---|---|---|---|
| `users` | `users` 1:N `long_term_bookings` | `applicant_id` → `users.user_id` | 每筆長期借用由一位使用者提出 |
| `classrooms` | `classrooms` 1:N `long_term_bookings` | `classroom_id` → `classrooms.classroom_id` | 每筆長期借用指定一間教室 |
| `sections` | `sections` 1:N `long_term_bookings` | `start_section_id`、`end_section_id` → `sections.section_id` | 每筆長期借用指定開始與結束節次 |
| `booking_statuses` | `booking_statuses` 1:N `long_term_bookings` | `status_id` → `booking_statuses.status_id` | 每筆長期借用保存目前狀態 |
| `bookings` | `long_term_bookings` 1:N `bookings` | `bookings.long_term_id` → `long_term_bookings.long_term_id` | 一筆長期借用可展開為多筆實際日期預約 |
