...
|---|---|---|---|---|---|---|---|
| 1 | `booking_id` | `INT` | 是 | PK | 自動編號 | `PRIMARY KEY AUTO_INCREMENT` | 單次預約識別碼 |
...
| 4 | `long_term_id` | `INT` | 否 | FK → `long_term_bookings.long_term_id` | `NULL` | 欄位具有值時，外鍵參照必須存在 | 對應長期借用；一般單次借用允許為空值 |
| 5 | `course_time_id` | `INT` | 否 | FK → `course_times.course_time_id` | `NULL` | 欄位具有值時，外鍵參照必須存在 | 對應固定課程；非課程借用允許為空值 |
...
| 10 | `status_id` | `INT` | 是 | FK → `booking_statuses.status_id` | 無 | `NOT NULL`、外鍵參照必須存在 | 目前審核狀態 |
